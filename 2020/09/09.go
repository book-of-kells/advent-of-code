...
/*
func getNumbersThatSumTo(numToSumTo int, dataArr []int) *[]int {
	for i, _ := range dataArr {
		go func(i int) *[]int{
			acc := 0
			for acc < numToSumTo {
				for j, elem := range dataArr[i:] {
					acc += elem
					if acc == numToSumTo {
						sumArr := dataArr[i:j+1]
						return &sumArr
					}
				}
			}
...
func innerSumTo(numToSumTo int, dataArr []int, startIdx int) *[]int {
	acc := 0
	for acc < numToSumTo {
		for j, elem := range dataArr[startIdx:] {
			acc += elem
			if acc == numToSumTo {
				sumArr := dataArr[startIdx:j+1]
				return &sumArr
			}
		}
	}
